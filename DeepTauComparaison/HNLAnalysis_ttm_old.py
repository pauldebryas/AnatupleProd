...
from CoffeaAnalysis.HNLAnalysis.helpers import delta_r, data_goodrun_lumi, import_stitching_weights
...
class HNLAnalysis_ttm_old(processor.ProcessorABC):
    ...
    def get_var_axis_pairs():
        ...
        sc_axis = hist.Bin("sc", r"rawDeepTauVSjet score", 30, 0., 1.)
        ...
        v_a_pairs = [
            ...
            ('mT_tautau', mt_axis),
            ('mT_l1MET', mt_axis),
            ('sc_l1DeepTau2p5', sc_axis),
            ('sc_l2DeepTau2p5', sc_axis),
            ('sc_l1DeepTau2p1', sc_axis),
            ('sc_l2DeepTau2p1', sc_axis),
        ]
    ...
    def process(self, events):
        ...
        if self.region == 'A':
            ...
            events['SelTau'] = events.Tau[(events.Tau.pt > cut_tau_pt) & (np.abs(events.Tau.eta) < cut_tau_eta) & (np.abs(events.Tau.dz) < cut_tau_dz) & (events.Tau.idDeepTau2017v2p1VSmu >= cut_tau_idVSmu) & (events.Tau.idDeepTau2017v2p1VSe >= cut_tau_idVSe) & (events.Tau.idDeepTau2017v2p1VSjet < cut_tau_idVSjet_high) & (events.Tau.idDeepTau2017v2p1VSjet >= cut_tau_idVSjet_low) & (events.Tau.decayMode != 5) & (events.Tau.decayMode != 6)]
        ...
        if self.region == 'B':
            ...
            # + remove decay mode 5 and 6 as suggested here: https://twiki.cern.ch/twiki/bin/viewauth/CMS/TauIDRecommendationForRun2
            events['SelTau'] = events.Tau[(events.Tau.pt > cut_tau_pt) & (np.abs(events.Tau.eta) < cut_tau_eta) & (np.abs(events.Tau.dz) < cut_tau_dz) & (events.Tau.idDeepTau2017v2p1VSmu >= cut_tau_idVSmu) & (events.Tau.idDeepTau2017v2p1VSe >= cut_tau_idVSe) & (events.Tau.idDeepTau2017v2p1VSjet >= cut_tau_idVSjet) & (events.Tau.decayMode != 5) & (events.Tau.decayMode != 6)]
    ...
    def saved_rawDeepTau2018v2p5VSjet_l1(self, events, Tau, out, ds):
        out[f'sc_l1DeepTau2p5'].fill(ds=ds, sc=ak.flatten(Tau.rawDeepTau2018v2p5VSjet), weight=events.genWeight)

    def saved_rawDeepTau2018v2p5VSjet_l2(self, events, Tau, out, ds):
        out[f'sc_l2DeepTau2p5'].fill(ds=ds, sc=ak.flatten(Tau.rawDeepTau2018v2p5VSjet), weight=events.genWeight)

    def saved_rawDeepTau2017v2p1VSjet_l1(self, events, Tau, out, ds):
        out[f'sc_l1DeepTau2p1'].fill(ds=ds, sc=ak.flatten(Tau.rawDeepTau2017v2p1VSjet), weight=events.genWeight)

    def saved_rawDeepTau2017v2p1VSjet_l2(self, events, Tau, out, ds):
        out[f'sc_l2DeepTau2p1'].fill(ds=ds, sc=ak.flatten(Tau.rawDeepTau2017v2p1VSjet), weight=events.genWeight)
    ...
    def analyse_ttm(self, events, out, ds, mode):
        ...
        out[f'sumw_3leptons'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_3leptons'][ds] += len(events_ttm)
        ...
        out[f'sumw_l1sel'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_l1sel'][ds] += len(events_ttm)
        ...
        out[f'sumw_l2sel'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_l2sel'][ds] += len(events_ttm)
        ...
        out[f'sumw_l3sel'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_l3sel'][ds] += len(events_ttm)

        events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon = self.bjet_veto(events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon)
        out[f'sumw_bjetveto'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_bjetveto'][ds] += len(events_ttm)

        events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon = self.charge_veto(events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon)
        out[f'sumw_chargeveto'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_chargeveto'][ds] += len(events_ttm)

        '''
        events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon = self.met_veto(events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon)
        out[f'sumw_metselection'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_metselection'][ds] += len(events_ttm)

        events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon = self.z_veto_ttl(events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon)
        out[f'sumw_zveto'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_zveto'][ds] += len(events_ttm)
        '''
        #computing corrections
        if mode != 'Data':
            pileup_corr = get_pileup_correction(events_ttm, 'nominal')* self.norm_factor
            sf_mu = compute_sf_mu(Sel_Muon)
            Trigger_eff_corr_mu = get_trigger_correction_mu(Sel_Muon)
            # as we use DeepTau2018v2p5, we only have tau NRJ corrections 
            sf_tau1 = compute_sf_tau_e(Sel_Tau1)
            sf_tau2 = compute_sf_tau_e(Sel_Tau2)
            events_ttm.genWeight = events_ttm.genWeight * sf_mu * Trigger_eff_corr_mu * sf_tau1 * sf_tau2 * pileup_corr

        out[f'sumw_corrections'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_corrections'][ds] += len(events_ttm)
        
        # Save histograms
        self.saved_leading_muon(events_ttm, Sel_Muon, out, ds)
        self.saved_leading_tau(events_ttm, Sel_Tau1, out, ds)
        self.saved_subleading_tau(events_ttm, Sel_Tau2, out, ds)
        self.saved_dilepton_mass_taul1_OS(events_ttm, Sel_Muon, Sel_Tau1, Sel_Tau2, out, ds)
        self.saved_MET(events_ttm, out, ds)
        self.saved_drl1l2(events_ttm, Sel_Muon, Sel_Tau1, out, ds) #save dr_mutau
        self.saved_pt_sum_l1l2l3(events_ttm, Sel_Muon, Sel_Tau1, Sel_Tau2, out, ds) #save pt_sum_mutautau
        self.saved_pt_sum_l1l2MET(events_ttm, Sel_Muon, Sel_Tau1, out, ds) #save pt_sum_mutauMET
        self.saved_mT_tautau(events_ttm, Sel_Tau1, Sel_Tau2, out, ds) #save mT_tautau
        self.saved_mT_l1MET(events_ttm, Sel_Muon, events_ttm.MET, out, ds) #save mT_muMET
        self.saved_rawDeepTau2018v2p5VSjet_l1(events_ttm, Sel_Tau1, out, ds)
        self.saved_rawDeepTau2018v2p5VSjet_l2(events_ttm, Sel_Tau2, out, ds)
        self.saved_rawDeepTau2017v2p1VSjet_l1(events_ttm, Sel_Tau1, out, ds)
        self.saved_rawDeepTau2017v2p1VSjet_l2(events_ttm, Sel_Tau2, out, ds)
