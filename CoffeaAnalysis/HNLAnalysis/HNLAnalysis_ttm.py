...
from CoffeaAnalysis.HNLAnalysis.helpers import select_lep1_IsoMu24, select_lep2, select_lep3, bjet_veto, charge_veto, met_veto, z_veto_ttl
from CoffeaAnalysis.HNLAnalysis.helpers import saved_leading_tau, saved_subleading_tau, saved_leading_muon, saved_dilepton_mass_taul1_OS, saved_MET, saved_drl1l2, saved_pt_sum_l1l2l3, saved_pt_sum_l1l2MET, saved_mT_tautau, saved_mT_l1MET

class HNLAnalysis_ttm(processor.ProcessorABC):
    ...
    def analyse_ttm(self, events, out, ds, mode):
        ...
        Sel_Muon = select_lep1_IsoMu24(events_ttm, min_dr_cut=0.2)
        ...
        Sel_Tau1 = select_lep2(events_ttm, Sel_Muon, type='tau', delta_r_cut = 0.5)
        ...
        Sel_Tau2 = select_lep3(events_ttm, Sel_Muon, Sel_Tau1, type='tau', delta_r_cut = 0.5)
        ...
        '''
        events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon = bjet_veto(events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon)
        out[f'sumw_bjetveto'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_bjetveto'][ds] += len(events_ttm)

        events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon = charge_veto(events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon)
        out[f'sumw_chargeveto'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_chargeveto'][ds] += len(events_ttm)

        events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon = met_veto(events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon)
        out[f'sumw_metselection'][ds] += ak.sum(events_ttm.genWeight)
        out[f'n_ev_metselection'][ds] += len(events_ttm)

        events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon = z_veto_ttl(events_ttm, Sel_Tau1, Sel_Tau2, Sel_Muon)
        ...
        # Save histograms
        saved_leading_muon(events_ttm, Sel_Muon, out, ds)
        saved_leading_tau(events_ttm, Sel_Tau1, out, ds)
        saved_subleading_tau(events_ttm, Sel_Tau2, out, ds)
        saved_dilepton_mass_taul1_OS(events_ttm, Sel_Muon, Sel_Tau1, Sel_Tau2, out, ds)
        saved_MET(events_ttm, out, ds)
        saved_drl1l2(events_ttm, Sel_Muon, Sel_Tau1, out, ds) #save dr_mutau
        saved_pt_sum_l1l2l3(events_ttm, Sel_Muon, Sel_Tau1, Sel_Tau2, out, ds) #save pt_sum_mutautau
        saved_pt_sum_l1l2MET(events_ttm, Sel_Muon, Sel_Tau1, out, ds) #save pt_sum_mutauMET
        saved_mT_tautau(events_ttm, Sel_Tau1, Sel_Tau2, out, ds) #save mT_tautau
        saved_mT_l1MET(events_ttm, Sel_Muon, events_ttm.MET, out, ds) #save mT_muMET
