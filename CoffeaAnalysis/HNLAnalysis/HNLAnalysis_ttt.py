...
from CoffeaAnalysis.HNLAnalysis.correction_helpers import get_pileup_correction, compute_sf_tau_e, get_trigger_correction_tau
from CoffeaAnalysis.HNLAnalysis.helpers import select_lep_DoubleMediumChargedIsoPFTauHPS35_Trk1_eta2p1_Reg, select_lep3, bjet_veto, charge_veto, met_veto
from CoffeaAnalysis.HNLAnalysis.helpers import saved_leading_tau, saved_subleading_tau, saved_subsubleading_tau, saved_MET, saved_drl1l2 

class HNLAnalysis_ttt(processor.ProcessorABC):
    ...
    def process(self, events):
       ...
       #stitching
        ...
        self.analyse_ttt(events, out, ds, mode)

        return out

    def analyse_ttt(self, events, out, ds, mode):

        # select ttt events: require 3 reco Tau 
        events_ttt = events[(ak.num(events.SelTau) == 3)]

        out[f'sumw_3leptons'][ds] += ak.sum(events_ttt.genWeight)
        out[f'n_ev_3leptons'][ds] += len(events_ttt)
        ...
        out[f'sumw_hlt'][ds] += ak.sum(events_ttt.genWeight)
        out[f'n_ev_hlt'][ds] += len(events_ttt)

        if len(events_ttt) == 0:
            return

        # select reco tau1 and tau2 that match HLT DoubleMediumChargedIsoPFTauHPS35_Trk1_eta2p1_Reg with dr(t2,t1)>0.5
        events_ttt, Sel_Tau1, Sel_Tau2 = select_lep_DoubleMediumChargedIsoPFTauHPS35_Trk1_eta2p1_Reg(events_ttt, min_dr_cut=0.5, min_pt_cut=40.)

        out[f'sumw_l1sel'][ds] += ak.sum(events_ttt.genWeight)
        out[f'n_ev_l1sel'][ds] += len(events_ttt)

        out[f'sumw_l2sel'][ds] += ak.sum(events_ttt.genWeight)
        out[f'n_ev_l2sel'][ds] += len(events_ttt)
        ...
        Sel_Tau3 = select_lep3(events_ttt, Sel_Tau1, Sel_Tau2, type='tau', delta_r_cut = 0.5)
        ...
        out[f'sumw_l3sel'][ds] += ak.sum(events_ttt.genWeight)
        out[f'n_ev_l3sel'][ds] += len(events_ttt)

        events_ttt, Sel_Tau1, Sel_Tau2, Sel_Tau3 = bjet_veto(events_ttt, Sel_Tau1, Sel_Tau2, Sel_Tau3)
        out[f'sumw_bjetveto'][ds] += ak.sum(events_ttt.genWeight)
        out[f'n_ev_bjetveto'][ds] += len(events_ttt)

        events_ttt, Sel_Tau1, Sel_Tau2, Sel_Tau3 = charge_veto(events_ttt, Sel_Tau1, Sel_Tau2, Sel_Tau3)
        out[f'sumw_chargeveto'][ds] += ak.sum(events_ttt.genWeight)
        out[f'n_ev_chargeveto'][ds] += len(events_ttt)

        events_ttt, Sel_Tau1, Sel_Tau2, Sel_Tau3 = met_veto(events_ttt, Sel_Tau1, Sel_Tau2, Sel_Tau3)
        out[f'sumw_metselection'][ds] += ak.sum(events_ttt.genWeight)
        out[f'n_ev_metselection'][ds] += len(events_ttt)

        # Save histograms
        saved_leading_tau(events_ttt, Sel_Tau1, out, ds)
        saved_subleading_tau(events_ttt, Sel_Tau2, out, ds)
        saved_subsubleading_tau(events_ttt, Sel_Tau3, out, ds)
        saved_MET(events_ttt, out, ds)
        saved_drl1l2(events_ttt, Sel_Tau1, Sel_Tau2, out, ds)
