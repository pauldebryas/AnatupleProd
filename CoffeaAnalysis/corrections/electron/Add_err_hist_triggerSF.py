...
Root_file_path = f'sf_el_{year}_HLT{HLT_name[year]}.root' 
new_root_file_path = f'sf_el_{year}_HLT{HLT_name[year]}_witherr.root'


correction_folder_path = f'{os.getenv("ANALYSIS_PATH")}/CoffeaAnalysis/corrections/electron/data/{year}/'
...
new_histogram_err = create_histogram_like_original(histogram, histogram.GetName() + "_err")
new_histogram_val = create_histogram_like_original(histogram, histogram.GetName())
