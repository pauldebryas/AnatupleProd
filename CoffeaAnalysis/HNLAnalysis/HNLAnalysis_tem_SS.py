...
class HNLAnalysis_tem_SS(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        self.analyse_tem_SS(events, out, ds, mode)

        return out

    def analyse_tem_SS(self, events, out, ds, mode):
        ...
        out[f'sumw_3leptons'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_3leptons'][ds] += len(events_tem_SS)

        # events should pass most efficient HLT (for now only 1: IsoMu24)
        events_tem_SS = events_tem_SS[events_tem_SS.HLT.IsoMu24]

        out[f'sumw_hlt'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_hlt'][ds] += len(events_tem_SS)

        if len(events_tem_SS) == 0:
            return

        # select reco muon that match HLT IsoMu24
        Sel_Muon = select_lep1_IsoMu24(events_tem_SS, min_dr_cut=0.2)

        #removing non matching events
        events_tem_SS = events_tem_SS[(ak.num(Sel_Muon) == 1)]
        Sel_Muon = Sel_Muon[(ak.num(Sel_Muon) == 1)]

        out[f'sumw_l1sel'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_l1sel'][ds] += len(events_tem_SS)

        # select reco Electron with dr(e,mu)>0.5 (in case there is more than 1 selected we choose the one with higher pt )
        Sel_Electron = select_lep2(events_tem_SS, Sel_Muon, type='electron', delta_r_cut = 0.5)
        ...
        out[f'sumw_l2sel'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_l2sel'][ds] += len(events_tem_SS)
        ...
        Sel_Tau = select_lep3(events_tem_SS, Sel_Muon, Sel_Electron, type='tau', delta_r_cut = 0.5)
        ...
        out[f'sumw_l3sel'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_l3sel'][ds] += len(events_tem_SS)
        '''
        events_tem_SS, Sel_Tau, Sel_Electron, Sel_Muon = bjet_veto(events_tem_SS, Sel_Tau, Sel_Electron, Sel_Muon)
        out[f'sumw_bjetveto'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_bjetveto'][ds] += len(events_tem_SS)

        events_tem_SS, Sel_Tau, Sel_Electron, Sel_Muon = charge_veto(events_tem_SS, Sel_Tau, Sel_Electron, Sel_Muon)
        out[f'sumw_chargeveto'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_chargeveto'][ds] += len(events_tem_SS)

        events_tem_SS, Sel_Tau, Sel_Electron, Sel_Muon = met_veto(events_tem_SS, Sel_Tau, Sel_Electron, Sel_Muon)
        out[f'sumw_metselection'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_metselection'][ds] += len(events_tem_SS)

        events_tem_SS, Sel_Electron, Sel_Tau, Sel_Muon = z_veto_tll(events_tem_SS, Sel_Electron, Sel_Tau, Sel_Muon)
        out[f'sumw_zveto'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_zveto'][ds] += len(events_tem_SS)
        '''
        #computing corrections
        if mode != 'Data':
            pileup_corr = get_pileup_correction(events_tem_SS, 'nominal')* self.norm_factor
            ...
            events_tem_SS.genWeight = events_tem_SS.genWeight * sf_mu * Trigger_eff_corr_mu * sf_e * sf_tau * pileup_corr

        out[f'sumw_corrections'][ds] += ak.sum(events_tem_SS.genWeight)
        out[f'n_ev_corrections'][ds] += len(events_tem_SS)  
        
        # Save histograms
        saved_leading_muon(events_tem_SS, Sel_Muon, out, ds)
        saved_leading_tau(events_tem_SS, Sel_Tau, out, ds)
        saved_leading_electron(events_tem_SS, Sel_Electron, out, ds)
        saved_MET(events_tem_SS, out, ds)
        saved_dilepton_mass(events_tem_SS, Sel_Tau, Sel_Electron, out, ds)
        saved_drl1l2(events_tem_SS, Sel_Muon, Sel_Electron, out, ds)
