...
from CoffeaAnalysis.HNLAnalysis.helpers import saved_leading_muon, saved_subleading_muon, saved_leading_tau, saved_dilepton_mass, saved_MET, saved_drl1l2
...
class HNLAnalysis_tmm(processor.ProcessorABC):
    ...
    def analyse_tmm(self, events, out, ds, mode):
        ...
        out[f'sumw_3leptons'][ds] += ak.sum(events_tmm.genWeight)
        out[f'n_ev_3leptons'][ds] += len(events_tmm)
        ...
        out[f'sumw_l2sel'][ds] += ak.sum(events_tmm.genWeight)
        out[f'n_ev_l2sel'][ds] += len(events_tmm)
        ...
        Sel_Tau = select_lep3(events_tmm, Sel_Muon, Sel_Muon2, type='tau', delta_r_cut = 0.5)
        ...
        out[f'sumw_l3sel'][ds] += ak.sum(events_tmm.genWeight)
        out[f'n_ev_l3sel'][ds] += len(events_tmm)
        '''
        events_tmm, Sel_Tau, Sel_Muon, Sel_Muon2 = bjet_veto(events_tmm, Sel_Tau, Sel_Muon, Sel_Muon2)
        out[f'sumw_bjetveto'][ds] += ak.sum(events_tmm.genWeight)
        out[f'n_ev_bjetveto'][ds] += len(events_tmm)

        events_tmm, Sel_Tau, Sel_Muon, Sel_Muon2 = charge_veto(events_tmm, Sel_Tau, Sel_Muon, Sel_Muon2)
        out[f'sumw_chargeveto'][ds] += ak.sum(events_tmm.genWeight)
        out[f'n_ev_chargeveto'][ds] += len(events_tmm)

        events_tmm, Sel_Tau, Sel_Muon, Sel_Muon2 = met_veto(events_tmm, Sel_Tau, Sel_Muon, Sel_Muon2)
        out[f'sumw_metselection'][ds] += ak.sum(events_tmm.genWeight)
        out[f'n_ev_metselection'][ds] += len(events_tmm)

        events_tmm, Sel_Tau, Sel_Muon, Sel_Muon2 = z_veto_tll(events_tmm, Sel_Tau, Sel_Muon, Sel_Muon2)
        out[f'sumw_zveto'][ds] += ak.sum(events_tmm.genWeight)
        out[f'n_ev_zveto'][ds] += len(events_tmm)
        '''
        ...
        # Save histograms
        saved_leading_muon(events_tmm, Sel_Muon, out, ds)
        saved_subleading_muon(events_tmm, Sel_Muon2, out, ds)
        saved_leading_tau(events_tmm, Sel_Tau, out, ds)
        saved_dilepton_mass(events_tmm, Sel_Muon, Sel_Muon2, out, ds)
        saved_MET(events_tmm, out, ds)
        saved_drl1l2(events_tmm, Sel_Muon, Sel_Muon2, out, ds)
